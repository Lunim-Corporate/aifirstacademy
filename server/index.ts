...
// Lazy-load certificatesRouter only when a request comes in
app.use("/api/certificates", (req, res, next) => {
  const certificatesRouter = require("./routes/certificates").default || require("./routes/certificates");
  return certificatesRouter(req, res, next);
});
  ...
  app.use(helmet({
    contentSecurityPolicy: process.env.NODE_ENV === 'production' ? {
      ...
      directives: {
        ...
        scriptSrc: ["'self'", "'unsafe-inline'"],           // allow Vite inline preamble
        imgSrc: ["'self'", "data:", "https:"],
        connectSrc: ["'self'", "https:", "ws:"],            // allow Vite HMR websocket
        fontSrc: ["'self'", "https://fonts.gstatic.com"],
        objectSrc: ["'none'"],
        mediaSrc: ["'self'"],
        frameSrc: ["'none'"],
  ...
  const generalLimiter = rateLimit({
    ...
    legacyHeaders: false,
    // Disable limiter in non-production or when explicitly disabled
    skip: () => process.env.NODE_ENV !== 'production' || process.env.RATE_LIMIT_DISABLE === 'true',
  });
