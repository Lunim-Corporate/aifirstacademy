...
export default function Sidebar({ currentPath, additionalItems = [] }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className="sm:hidden fixed top-16 left-0 right-0 z-50 bg-background border-b border-gray-200 dark:border-gray-700/40">
        <Button
          variant="ghost"
          size="sm"
      ...
      <aside className={`${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0 fixed sm:static top-[4rem] sm:top-0 left-0 z-50 sm:z-auto w-64 bg-muted/30 border-r border-gray-200 dark:border-gray-700/40 h-[calc(100vh-4rem)] sm:h-full overflow-y-auto transition-transform duration-300`}>
        <SidebarContent />
      </aside>
    </>
  );
